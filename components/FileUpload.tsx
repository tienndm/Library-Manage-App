...
const ImageUpload = ({
    ...
    const styles = {
        button: variant === 'dark' ? 'bg-dark-300' : 'bg-light-600 border-gray-100',
        placeholder: variant === 'dark' ? 'text-light-300' : 'text-slate-500',
    ...
    const onValidate = (file: File) => {
        if (type === 'image') {
            if (file.size > 20 * 1024 * 1024) {
                toast('Image size should be less than 20MB');
                return false;
            }
        } else if (type === 'video') {
            if (file.size > 50 * 1024 * 1024) {
                toast('Video size should be less than 50MB');
                return false;
            }
        }
        return true;
    ...
    return (
        ...
        >
            ...
            >
                ...
                {file && <p className={cn('upload-filename', styles.text)}>
                    {file.filePath}
                </p>}
            </button>
